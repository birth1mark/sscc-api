...
function handlePrefix(url) {
    ...
    else return error(`Invalid length: expected 18 digits, got ${digits.length}`);
...
const VALID_GS1_RANGES = [
    [1,19],[30,39],[60,139],[200,299],[300,379],
    [380,380],[383,383],[385,385],[387,387],[389,390],
    [400,440],[450,459],[490,499],[460,469],[470,489],
    [500,509],[520,521],[528,531],[535,535],[539,539],
    [540,549],[560,560],[569,569],[570,579],
    [590,590],[594,594],[599,599],[600,632],
    [640,649],[680,681],[690,699],
    [700,709],[729,729],[730,739],[740,746],
    [750,750],[754,755],[758,759],[760,790],
    [800,839],[840,849],[850,850],[858,860],
    [865,865],[867,869],[870,879],[880,881],
    [883,885],[888,888],[890,890],[893,894],
    [896,896],[899,899],[900,919],[930,939],
    [940,949],[950,952],[955,955],[958,958],
    [977,984],[990,999],
];
...
function tryAddSSCC(raw, found) {
    if (!raw) return;
    const digits = raw.replace(/\D/g, '');
    let sscc18;
    if (digits.length === 20 && digits.startsWith('00')) sscc18 = digits.substring(2);
    else if (digits.length === 18) sscc18 = digits;
    else if (digits.length === 17) {
        if (isTrivialSequence(digits)) return;
        const full = digits + getCD(digits);
        if (!isValidGS1Prefix(full)) return;
        if (found.some(f => f.sscc === full)) return;
        const cd = getCD(digits);
        const p  = getPrefix(full);
        found.push({ sscc: full, formatted: `(00)${full}`, valid: true, generated: true,
            checkDigit: cd, gs1: { prefix: p.prefix, country: p.name, countryCode: p.code, flag: p.flag } });
        return;
    } else return;

    if (isTrivialSequence(sscc18))   return;
    if (!isValidGS1Prefix(sscc18))   return;
    if (found.some(f => f.sscc === sscc18)) return;
    ...
    segments.forEach(seg => {
        ...
        if (tag === 'GIN') {
            elements.slice(1).forEach(el => {
                ...
                if (parts[0] === 'BJ' && parts[1]) tryAddSSCC(parts[1], found);
                else if (parts.length === 1)        tryAddSSCC(parts[0], found);
            });
        }
        if (tag === 'RFF') {
            elements.slice(1).forEach(el => {
                const parts = el.split(compSep);
                if ((parts[0] === 'SI' || parts[0] === 'AAK') && parts[1]) tryAddSSCC(parts[1], found);
            });
        }
        if (tag === 'PAC') {
            elements.slice(1).forEach(el => el.split(compSep).forEach(p => tryAddSSCC(p, found)));
        }
    });
    return found;
}
...
function parseXML(text) {
    const found = [];
    const knownTags = ['EXIDV','EXIDV2','VHILM_KU','SSCC','NVE','GIN','ShipmentContainerId','ContainerId','PackageId','PalletId','HandlingUnitId'];
    knownTags.forEach(tag => {
        const re = new RegExp(`<${tag}[^>]*>([^<]+)</${tag}>`, 'gi');
        let m;
        while ((m = re.exec(text)) !== null) tryAddSSCC(m[1].trim(), found);
    });
    extractGeneric(text.replace(/<[^>]+>/g, ' '), found);
    return found;
}

function extractGeneric(text, found) {
    let m;
    const reAI = /\(00\)(\d{18})/g;
    while ((m = reAI.exec(text)) !== null) tryAddSSCC(m[1], found);
    const re20 = /(?<!\d)(00\d{18})/g;
    while ((m = re20.exec(text)) !== null) tryAddSSCC(m[1], found);
    const re18 = /(?<!\d)(\d{18})(?!\d)/g;
    while ((m = re18.exec(text)) !== null) tryAddSSCC(m[1], found);
}

function parseGeneric(text) {
    const found = [];
    extractGeneric(text, found);
    return found;
}
...
function parseText(text) {
    ...
    if (results.length === 0 && fmt !== 'generic') results = parseGeneric(text);
...
async function handleExtract(request) {
    ...
    if (!text || !text.trim()) return error('Empty body — send the file content as plain text in the request body');
    ...
    return json({
        ...
        summary: { valid, invalid, generated: gen },
        items:   results,
    });
}
...
export default {
    async fetch(request, env) {
        ...
        // POST /extract accepts file content as plain text body
            ...
            default:
                return json({
                    ...
                    endpoints: {
                        ...
                        extract:  'POST /extract (body: EDIFACT, XML, CSV or TXT content)',
                        health:   '/health',
                    },
                });
        }
